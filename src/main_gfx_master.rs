...
use hal::{buffer, command, format as f, image as i, 
          memory as m, pass, pso, pool, window::Extent2D};
use hal::{Device, PhysicalDevice, Surface, Swapchain}; // Instance, 
use hal::{DescriptorPool, FrameSync, Primitive,
...
pub use setup::display;
#[cfg(any(feature = "vulkan", feature = "dx12", feature = "metal", feature = "gl"))]
pub use setup::display::{DIMS, COLOR_RANGE, ENTRY_NAME, };
pub use util::rvertex::Vertex;
...
fn main() {
  ...
  // instantiate backend
  #[cfg(not(feature = "gl"))]
  let (mut events_loop, window, _instance, mut adapters, mut surface) = display::make_non_gl_backend("raumEn");
  #[cfg(feature = "gl")]
  let (mut events_loop, mut adapters, mut surface) = display::make_gl_backend("raumEn");
  ...
  let buffer_len = spaceship_mesh.verts.len() as u64 * buffer_stride;
  ...
  // TODO: check transitions: read/write mapping and vertex buffer read
  {
    let mut vertices = device
      ...
      .unwrap();
    vertices.copy_from_slice(&spaceship_mesh.verts);
    device.release_mapping_writer(vertices);
  }
  
  // Image
  let img_data = include_bytes!("res/img/spaceship.png");
  ...
  // try to create transform buffer
  
  let transform = util::TransMat::new();
  let tbuffer_len = std::mem::size_of::<util::TransMat>() as u64;
  
  let tbuffer_unbound = device
    .create_buffer(tbuffer_len, buffer::Usage::VERTEX)
    .unwrap();
  let tbuffer_req = device.get_buffer_requirements(&tbuffer_unbound);
  ...
  let tbuffer_memory = device
    .allocate_memory(upload_type, tbuffer_req.size)
    .unwrap();
  let transform_buffer = device
    .bind_buffer_memory(&tbuffer_memory, 0, tbuffer_unbound)
    .unwrap();

  // TODO: check transitions: read/write mapping and vertex buffer read
  {
    let mut transmat = device
      .acquire_mapping_writer::<util::TransMat>(&tbuffer_memory, 0..tbuffer_len)
      .unwrap();
    transmat.copy_from_slice(&[transform]);
    device.release_mapping_writer(transmat);
  }
  ...
  device.write_descriptor_sets(vec![
    ...
    //I think this is where we bind our uniform, have to figure out how to create the buffer
    pso::DescriptorSetWrite {
      set: &desc_set,
      binding: 2,
      array_offset: 0,
      descriptors: Some(pso::Descriptor::Buffer(&transform_buffer, )),
    },
  ]);
...
fn swapchain_stuff(
  ...
  let pipeline = {
      let vs_module = {
        let glsl = fs::read_to_string("src/shaders/model.vert").unwrap();
      ...
      let fs_module = {
        let glsl = fs::read_to_string("src/shaders/model.frag").unwrap();
